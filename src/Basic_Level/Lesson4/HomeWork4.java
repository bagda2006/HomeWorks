...
public class HomeWork4 {
    ...
    public static List<String> correctedList(List<String> stringList) {
        ...
        for(int i = 0;i < stringList.size();i++){
            for(int j = 0;j < stringList.get(i).length();j++){
                if(stringList.get(i).charAt(j) >= 'A' && stringList.get(i).charAt(j) <= 'Z') {
                    flag = true;
                    break;
                }
            }
    ...
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
        HashMap <String, Integer> newShopCart = new HashMap<>();
        for (Map.Entry<String, Integer> item:shopCart.entrySet()){
            //я сделал цикл foreach по объектам в коллекции HashMap shopCart
            //entrySet() - возвращает набор всех элементов Map.Entry<String, Integer>
            //Map.Entry - представляет объект с ключом типа String и значением типа Integer
            if (item.getValue() <= 300) {
                //getValue() - возвращает значение элемента(объекта) из коллекции
                newShopCart.put(item.getKey(), item.getValue());
                //getKey() - возвращает ключ элемента(объекта) из коллекции
            }
        }
        return newShopCart;
    }
